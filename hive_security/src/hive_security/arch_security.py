...
- **单一事实源**：循环依赖检测复用包内内置 ``_validate_dependency_graph``，
  不重复实现环检测算法；敏感模式复用包内 ``_DEFAULT_MASK_PATTERNS``。
- **确定性 id**：``F-{threat_id}-{n:03d}``，按每威胁出现次序编号，同输入必同输出。

依赖：仅标准库（dataclasses/re/json/pathlib）+ 同包 ``hive_security.threat_model``
（ThreatCatalog/ValidationPolicy/apply_policy）。本包零外部运行时依赖
（不依赖 pydantic / langchain）。
"""
...
from .cwe_map import CWE_MAP, OWASP_LLM_MAP
...
# 内置敏感数据脱敏模式（与 agent-hive data_compliance.DEFAULT_MASK_RULES 同源）：
# base64 长串 / 密钥串 / 银行卡 / SSN / 邮箱，共 5 条。
_DEFAULT_MASK_PATTERNS: list[str] = [
    r'[A-Za-z0-9+/]{20,}={0,2}',   # 疑似 API Key 的 base64 字符串
    r'[A-Za-z0-9_\-]{20,}',        # 疑似密钥的字符串
    r'\b\d{17,19}\b',              # 疑似银行卡/信用卡号
    r'\b\d{3}-\d{2}-\d{4}\b',      # 疑似美国 SSN
    r'\b[\w\.-]+@[\w\.-]+\.\w+\b',  # 邮箱地址（低风险）
]
...
class SecurityReport:
    ...
    def to_sarif(self) -> str:
        ...
        每个 result 携带 ``properties``：``{"cwe": [...], "owasp_llm_top10": [...]}``，
        映射见 ``hive_security.cwe_map``；未知 threat_id 给空列表，不崩溃。
        """
        results = []
        for f in self.findings:
            results.append({
                ...
                "properties": {
                    "cwe": CWE_MAP.get(f.threat_id, []),
                    "owasp_llm_top10": OWASP_LLM_MAP.get(f.threat_id, []),
                },
            })
        sarif = {
            "$schema": (
...
def _validate_dependency_graph(packages: list[dict]) -> None:
    """校验依赖图（包内内置实现，语义与 agent-hive scheduler.validate_dependency_graph 一致）。

    规则：非空；包 id 全局唯一；``depends_on`` 引用的 id 必须存在；无环（三色 DFS）。
    违规一律抛 ``ValueError``；环检测错误信息含「环」，
    供 ``check_dependency_cycle`` 判定是否产出 T-PATT-1。
    """
    if not packages:
...
def _mask_sensitive(text: str) -> str:
    """复用包内 _DEFAULT_MASK_PATTERNS 对文本脱敏（确定性）。"""
    masked = text
    for pattern in _DEFAULT_MASK_PATTERNS:
...
def check_dependency_cycle(architecture: dict) -> list[SecurityFinding]:
    """循环依赖检查：投影 modules → packages，复用包内 _validate_dependency_graph。
    ...
    try:
        _validate_dependency_graph(packages)
...
def check_dist_artifacts(
    ...
    - 硬编码密钥：复用包内 ``_DEFAULT_MASK_PATTERNS``
    ...
    """
    patterns = list(mask_patterns) if mask_patterns is not None else list(_DEFAULT_MASK_PATTERNS)
    ...
    for path in sorted(root.rglob("*")):
        ...
        if any(p.search(text) for p in compiled):
            findings.append(SecurityFinding(
                ...
                category="information_disclosure",
                severity="high",
                evidence=f"疑似硬编码密钥：{rel}（命中内置脱敏模式）",
